...
class AppStore extends Component {
  state = {
    activeAppId: appsList[0].category,
    inputAppSearch: '',
  }
  ...
  onEventChange = event => {
    this.setState({inputAppSearch: event.target.value})
  }

  searchByCategory = inputAppSearch => {
    const categorySearch = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputAppSearch.toLowerCase()),
    )
  ...
  render() {
    const {inputAppSearch} = this.state
    const searchResults = this.searchByCategory(inputAppSearch)
    const {activeAppId} = this.state
    const upadatedCategoryItem = searchResults.filter(
      eachApp => eachApp.category === activeAppId,
    )
    return (
      <div className="bg-container">
        ...
        <ul className="tabs-list-continer">
          {tabsList.map(eachTab => (
            <TabItem
              tabsList={eachTab}
              key={eachTab.tabId}
              getItemByCategory={this.getItemByCategory}
              isActive={eachTab.tabId === activeAppId}
            />
          ))}
        </ul>
        <ul className="app-item-container">
          {upadatedCategoryItem.map(eachApp => (
